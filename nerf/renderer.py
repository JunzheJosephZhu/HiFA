...
try:
    ...
    print("cannot use meshutils")
from .utils import custom_meshgrid, safe_normalize
from diffusers.models.vae import DiagonalGaussianDistribution
import matplotlib.pyplot as plt

def sample_pdf(bins, weights, n_samples, det=False, strategy='boxcar'):
    # This implementation is from NeRF
    # bins: [B, T], old_z_vals
    # weights: [B, T - 1], bin weights.
    # return: [B, n_samples], new_z_vals

    # Get pdf * delta(or pmf)
    weights = weights + 1e-5  # prevent nans
    pmf = weights / torch.sum(weights, -1, keepdim=True)
    # kernel smoothing
    if strategy == "boxcar":
        pmf = pmf / 3 + torch.roll(pmf, -1, dims=-1) / 3 + torch.roll(pmf, 1, dims=-1) / 3
    elif strategy == "trapezoidal":
        pmf = pmf * 0.75 + torch.roll(pmf, -1, dims=-1) * 0.125 + torch.roll(pmf, 1, dims=-1) * 0.125
...
class NeRFRenderer(nn.Module):
        ...
        def _export(v, f, h0=2048, w0=2048, ssaa=1, name=''):
            ...
            if mask.any():
                ...
                while head < xyzs.shape[0]:
                    ...
                    self.training = True
                    self.opt.dir_rate = 0.0
                    _, albedo, _ = self.common_forward(xyzs[head:tail], torch.zeros_like(xyzs[head:tail]))
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3), rays_d.reshape(-1, 1, 3).expand_as(xyzs).reshape(-1, 3))
        ...
        num_levels = 1
        upsample_steps = upsample_steps // num_levels
        for level in range(num_levels):
            ...
            if upsample_steps > 0:
                with torch.no_grad():
                    ...
                    new_z_vals = sample_pdf(z_vals_mid, weights[:, 1:-1], upsample_steps, det=not self.training, strategy=self.opt.sampling_strategy).detach() # [N, t]
                    
                    # alternatively: integrate at bin boundaries, use average weight of left and right bin as PMF
                    # new_z_vals = sample_pdf(z_vals, weights[:, :-1] / 2 + weights[:, 1:] / 2, upsample_steps, det=not self.training).detach() # [N, t]

                    new_xyzs = rays_o.unsqueeze(-2) + rays_d.unsqueeze(-2) * new_z_vals.unsqueeze(-1) # [N, 1, 3] * [N, t, 1] -> [N, t, 3]
                    new_xyzs = torch.min(torch.max(new_xyzs, aabb[:3]), aabb[3:]) # a manual clip.

                # only forward new points to save computation
                new_density_outputs = self.density(new_xyzs.reshape(-1, 3), rays_d.reshape(-1, 1, 3).expand_as(new_xyzs).reshape(-1, 3))
        ...
        transmittance = 1 - alphas
        pdf = weights / deltas.clamp(min=1e-5)
        # pdf = pdf / torch.sum(pdf, dim=-1, keepdim=True).clamp(min=1e-5)
        ...
        rgbs = rgbs.view(N, -1, rgbs.size(-1)) # [N, T+t, 3] or [N, T+t, 4] or [N, T+t, 8]
        ...
        # calculate color
        image = torch.sum(weights.unsqueeze(-1) * rgbs, dim=-2) # [N, 3] or [N, 4], in [0, 1]

        # normal_img
        if self.opt.return_normal:
            normal_img = torch.sum(weights.unsqueeze(-1) * normals, dim=-2) # [N, 3] or [N, 4], in [0, 1]
        ...
        if self.opt.return_normal:
            normal_img = normal_img + (1 - weights_sum).unsqueeze(-1) * bg_color          

        image = image.view(*prefix, image.size(-1)).clamp(0, 1)
        depth = depth.view(*prefix)
        weights_sum = weights_sum.view(*prefix)
        sigmas = sigmas.view(*prefix, -1)
        if self.opt.return_normal:
            normal_img = normal_img.view(*prefix, image.size(-1)).clamp(0, 1)
        ...
        results['pdf'] = pdf
        results['bounds'] = nears, fars
        if self.opt.return_normal:
            results['normals'] = normal_img
        return results
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096, **kwargs):
        ...
        if staged and not self.cuda_ray:
            ...
            image = torch.empty((B, N, rgb_dim), device=device)
            weights_sum = torch.empty((B, N), device=device)
            weights = torch.empty((B, N, self.opt.num_steps + self.opt.upsample_steps), device=device)
            zvals = torch.empty((B, N, self.opt.num_steps + self.opt.upsample_steps), device=device)
            sigmas = torch.empty((B, N, self.opt.num_steps + self.opt.upsample_steps), device=device)
            pdfs = torch.empty((B, N, self.opt.num_steps + self.opt.upsample_steps), device=device)
            if self.opt.return_normal:
                normals = torch.empty((B, N, rgb_dim), device=device)
            for b in range(B):
                head = 0
                while head < N:
                    ...
                    zvals[b:b+1, head:tail, :] = results_['z_vals']
                    pdfs[b:b+1, head:tail, :] = results_['pdf']                    
                    sigmas[b:b+1, head:tail, :] = results_['sigmas']
